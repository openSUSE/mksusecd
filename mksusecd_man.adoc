...
The *--kernel* option accepts a mix of kernel packages and kernel firmware packages.
That is, you can update both kernel firmware and kernel modules. But kernel
firmware is typically not kernel version dependent (it's in a common '/usr/lib/firmware' tree).

Note that mksusecd will not add all kernel firmware files but only those that are required by
the kernel modules used.

There are two cases: 1. the 'normal' case and 2. *--rebuild-initrd* is used.

1. New kernel modules / firmware files are added to the initrd. This means your initrd contains
...
2. Old kernel modules / firmware files are removed and only files from packages specified
in *--kernel* are used. This makes the initrd smaller and exactly reproduces the original
initrd layout. +
...
To mitigate this, as a special case if there are no kernel firmware packages
specified in *--kernel*, then the old kernel firmware files are kept.
